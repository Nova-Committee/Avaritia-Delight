...
public class BuddingBlazeTomatoBlock extends BuddingBushBlock implements BonemealableBlock
{
    public BuddingBlazeTomatoBlock(Properties properties) {
        super(properties);
    }
    ...
    public BlockState updateShape(BlockState state, Direction facing, BlockState facingState, LevelAccessor level, BlockPos currentPos, BlockPos facingPos) {
        if (state.getValue(BuddingBushBlock.AGE) == 4) {
            level.setBlock(currentPos, ADBlocks.blaze_tomatoes.get().defaultBlockState(), 3);
        }
    ...
    public void growPastMaxAge(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        level.setBlockAndUpdate(pos, ADBlocks.blaze_tomatoes.get().defaultBlockState());
    }
    ...
    public void performBonemeal(ServerLevel level, RandomSource random, BlockPos pos, BlockState state) {
        ...
        if (ageGrowth <= maxAge) {
            ...
            level.setBlockAndUpdate(pos, ADBlocks.blaze_tomatoes.get().defaultBlockState().setValue(BlazeTomatoBlock.VINE_AGE, remainingGrowth));
        }
    }
}